...
for (const exports of ['./index.js', { import: './index.js' }, { '.': { browser: { import: './index.js' } } }]) {
  test(`resolves root exports ${JSON.stringify(exports)}`, async (t) => {
    const { pkg } = await fixture(t, { exports });
    assert.equal((await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath: '.' })).status, 200);
  });
}

test('unknown and null subpaths cannot fall back to main', async (t) => {
  const { pkg } = await fixture(t, { exports: { '.': './index.js', './private': null }, main: './index.js' });
  for (const exportPath of ['./missing', './private']) {
    assert.equal((await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath })).status, 404);
  }
});

test('nested nonmatching conditions fall through, but explicit null remains blocked', async (t) => {
  const { pkg } = await fixture(t, { exports: { browser: { require: './unused.cjs' }, import: './index.js' } });
  assert.equal((await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath: '.' })).status, 200);
...
test('wildcard nested subpaths and exact blocked overrides resolve consistently', async (t) => {
  const { pkg } = await fixture(t, { exports: { './features/*': './features/*.js', './features/private': null } });
  await mkdir(path.join(pkg, 'features'));
  await writeFile(path.join(pkg, 'features/hello.js'), 'export default 1');
  assert.equal((await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath: './features/hello' })).status, 200);
  assert.equal(
    (await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath: './features/private' })).status,
    404
  );
});
...
test('wildcard captures cannot traverse to a blocked sibling export', async (t) => {
  ...
  for (const exportPath of ['./features/../private', './features/./../private', './features/x/../private']) {
    assert.equal(
      (await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath })).status,
      404,
...
test('array export targets fall back past invalid entries', async (t) => {
  const { pkg } = await fixture(t, { exports: ['../invalid.js', './index.js'] });
  assert.equal((await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath: '.' })).status, 200);
});
...
test('rejects path traversal and symlinks leaving a package', async (t) => {
  ...
  assert.equal((await sendEntry({ cdnPkgPath: pkg, fullName: 'demo@1', exportPath: '.' })).status, 403);
